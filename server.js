 ...
 *
 * Uses the current @google/genai SDK (the unified Google Gen AI SDK).
 * The legacy @google/generative-ai package is no longer used anywhere
 * in this file.
 */
...
/*
 * NOTE: Newer Google AI Studio keys can begin with "AQ." instead of
 * the older "AIzaSy" prefix. Both are valid Gemini API keys, so we
 * do NOT validate the key's prefix/shape here — we just confirm one
 * is present and let the Gemini API itself be the source of truth
 * on whether the key is accepted.
 */
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      /*
       * Log the full error server-side for debugging, but never
       * echo the API key or any request headers/credentials.
       */
